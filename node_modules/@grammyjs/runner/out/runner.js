 ...
 */
function run(bot, concurrency = 500, sourceOptions = {}, runnerOptions = {}, sinkOptions = {
    timeout: Infinity,
    timeoutHandler: () => { },
}) {
    var _a, _b;
    (_a = runnerOptions.maxRetryTime) !== null && _a !== void 0 ? _a : (runnerOptions.maxRetryTime = 15 * 60 * 60 * 1000); // 15 hours in milliseconds
    (_b = runnerOptions.retryInterval) !== null && _b !== void 0 ? _b : (runnerOptions.retryInterval = "exponential");
    // create update fetch function
    const fetchUpdates = createUpdateFetcher(bot, runnerOptions.maxRetryTime, runnerOptions.retryInterval, sourceOptions, runnerOptions.silent);
    // create source
    const source = source_js_1.createSource({
    ...
    // create sink
    const sink = sink_js_1.createConcurrentSink({ consume: (update) => bot.handleUpdate(update) }, async (error) => {
        try {
 ...
 */
function createUpdateFetcher(bot, maxRetryTime, retryInterval, sourceOptions, silent = false) {
